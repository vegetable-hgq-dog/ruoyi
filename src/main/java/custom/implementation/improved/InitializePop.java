...
public class InitializePop implements PopulationProducer {
    ...
    public InitializePop(List<Integer> lower, List<Integer> upper, List<Integer> suppliersNum, List<Integer> demand, List<Double> dq, List<Double> de, int cr, double deliveryDate, List<Double> tp, List<Double> transportDate, List<AbstractObjectiveFunction> objectives, Chromosome originalSolution) {
        this.lower = lower;
        this.upper = upper;
    ...
    public List<Chromosome> localSearchInitialize(Chromosome originalSolution, int localSearchSize, int populationSize){
        ...
        //初始解不满足时需要修复
        if (!(constraintFactor.satisfyDemand(originalChromosomeCopy,suppliersNum,demand)) || !(constraintFactor.satisfyOrderFulfillment(originalChromosomeCopy,suppliersNum,demand,dq,de,cr))){
            originalChromosomeCopy = new RepairMechanism().repair1(originalSolution, lower, upperOrder, suppliersNum, demand,dq,de,cr,deliveryDate,tp,transportDate);
        }
        ...
        do {
            for (int i = 0; i < first; i++){
                if (suppliersChoose.get(i)){
                    originalChromosomeCopy.set(i,new IntegerAllele(this.orderGenerate(i, lower, upperOrder)));
                }else {
                    originalChromosomeCopy.set(i,new IntegerAllele(0));
                }
            }
